...
class Solution: 
    def sametree(self,root,p):
        if root is None and p is None: return True
        elif root is None or p is None: return False
        else:
            if root.val == p.val:
                return self.sametree(root.left,p.left) and self.sametree(root.right,p.right) 
            else:
                return False#self.sametree(root.left,p.left) and self.sametree(root.right,p.right) 
