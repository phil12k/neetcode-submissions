class Solution:
    def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
        ...
        #dfs to visit every node in graph
        def dfs(i,j,visited,prev):
            #boundary condition if ode already visited in grap or fails higher to lower condition
            if i<0 or j<0 or i>=row or j>= col or (i,j) in visited or heights[i][j] < prev:
                return
            #add the node value and visit its neighbors
            visited.add((i,j))
